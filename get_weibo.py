...
class Weibo(object):
    def __init__(self,
                 ...
                 since_date='2019-02-25', # 起始时间，即爬取发布日期从该值到现在的微博，形式为yyyy-mm-dd
    ...
    def parse_weibo(self, weibo_info):
        ...
        selector = etree.HTML(text_body)  # 这个etree很强大，解析HTML很棒！
                    ...
                    if w['card_type'] == 9:
                        ...
                        if wb:
                            if wb['id'] in self.weibo_id_list:
                                continue
                            created_at = datetime.strptime(wb['created_at'],"%a %b %d %H:%M:%S %z %Y")
                            created = created_at.strftime("%Y-%m-%d")
                            create = datetime.strptime(created,"%Y-%m-%d")
                            since_date = datetime.strptime(self.since_date,"%Y-%m-%d")
                            until_date = datetime.strptime(self.until_date,"%Y-%m-%d")
                            if create < since_date:
                                if self.is_pinned_weibo(w):
                                    continue
                                else:
                                    return True                   
                            if create > until_date:
                                continue
    ...
    def get_write_info(self, wrote_count):
        ...
        for w in self.weibo[wrote_count:]:
            wb = OrderedDict()  # TODO OrderedDict
            for k, v in w.items():
    ...
    def get_result_headers(self):
        """获取要写入结果文件的表头"""
        result_headers = [
            '博文id', '正文', '原始图片url', '视频url', '位置', '日期', '工具', '点赞数', '评论数',
            '转发数', '话题', '@用户'
        ]
        if not self.filter:
...
def main():
    try:
        ...
        filter = 0  # 值为0表示爬取全部微博（原创微博+转发微博），值为1表示只爬取原创微博
        since_date = '2001-08-20'  # 起始时间，即爬取发布日期从该值到现在的微博，形式为yyyy-mm-dd
        until_date = '2021-09-20'
        pic_download = 0  # 值为0代表不下载微博原始图片,1代表下载微博原始图片
        video_download = 0  # 值为0代表不下载微博视频,1代表下载微博视频

        wb = Weibo(filter, since_date, until_date, pic_download, video_download)

        user_id_list = ['7635233503']
